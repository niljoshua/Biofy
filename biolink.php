...
<div class="row">
    <div class="col-12 col-xl-6">
        ...
        <div id="countdown-message" style="position: fixed; bottom: 10px; right: 10px; background-color: #f0f0f0; padding: 10px; border-radius: 5px; display: none; color: black; z-index: 50;">
            Atualizando o conteudo em: <span id="countdown-timer">2</span> segundos
        </div>
...
<script>
    ...
    let submitCount = 0; // Inicializa o contador
    let timeoutId; // Variável para armazenar o ID do timeout
    let countdownInterval; // Variável para armazenar o ID do intervalo de contagem regressiva

    // Função para mostrar a mensagem de contagem regressiva
    function showCountdown(seconds) {
        $('#countdown-message').show();
        updateCountdownMessage(seconds);
    ...
    // Função para atualizar a mensagem de contagem regressiva
    function updateCountdownMessage(seconds) {
        const message = `Atualizando o conteudo em: ${seconds} segundos`;
        $('#countdown-message').html(message);
    }

    // Evento de colapso
    $('[id^="biolink_block_expanded_content"]').on('show.bs.collapse.namespace', event => {
        let update_form_content = event.currentTarget;

        // Evento quando o foco sai do botão
        $('button.pcr-button').on('focusout.namespace', event => {
            console.log("Clique fora do botão!");
            $('form[name="update_biolink"], form[name="update_biolink_"]').submit();
            submitCount++; // Incrementa o contador
            console.log(`Formulário enviado ${submitCount} vezes`);
        });

        // Evento para qualquer mudança nos formulários com um delay de 2 segundos
        $('form[name="update_biolink"], form[name="update_biolink_"]').on('change.namespace', event => {
            clearTimeout(timeoutId); // Limpa o timeout anterior
            clearInterval(countdownInterval); // Limpa a contagem regressiva anterior
            showCountdown(2); // Mostra a contagem regressiva de 2 segundos
    ...
    // Adiciona um event listener para mudanças em qualquer elemento dentro do formulário com delay de 2 segundos
    $('#update_biolink').on('change.namespace', function() {
        clearTimeout(timeoutId); // Limpa o timeout anterior
        clearInterval(countdownInterval); // Limpa a contagem regressiva anterior
        showCountdown(2); // Mostra a contagem regressiva de 2 segundos
    ...
    // Form handling update
    $('form[name="update_biolink"],form[name="update_biolink_"]').on('submit.namespace', event => {
        ...
        $.ajax({
            ...
            error: () => {
                enable_submit_button(event.currentTarget.querySelector('[type="submit"][name="submit"]'));
                display_notifications('Ocorreu um erro. Por favor, tente novamente.', 'error', notification_container);
            },
        });
    });
});
